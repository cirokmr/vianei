        ...
        {projetos.map((projeto) => (
          <li key={projeto.id}>
            <Link href={`/projetos/${projeto.slug}`} className="group block py-8">
              <span className="text-eyebrow tracking-[0.14em] text-musgo uppercase">
                {projeto.situacao === "ativo" ? "Em andamento" : "Concluído"}
              </span>
              <span className="mt-2 block font-display text-[clamp(1.5rem,1.1rem+1.4vw,2.4rem)] leading-tight group-hover:text-pinhao">
                {projeto.titulo}
              </span>
